...
# Türkçe Karakter Desteği İçin Font Tanımlamaları
...
def generate_pdf_report(job: TrainingJob, tasks: list[ModelTaskResult]):
    buffer = io.BytesIO()
    ...
    meta_text = f"<b>İş Kimliği (Job ID):</b> {job.id}<br/><b>Veri Seti Dosyası:</b> {job.filename}<br/><b>Hedef Değişken (Target):</b> {job.target_column}"
    story.append(Paragraph(meta_text, subtitle_style))
    story.append(Spacer(1, 5))

    # 2. İstatistik Özet Kartları
    completed_tasks = [t for t in tasks if t.status == 'SUCCESS']
    best_model, best_r2 = None, -float('inf')
    for t in completed_tasks:
        if t.metrics and 'r2' in t.metrics and t.metrics['r2'] > best_r2:
    ...
    try:
        ...
        if model_path and os.path.exists(model_path):
            ...
            if hasattr(model_obj, 'feature_importances_'):
                ...
                feats = [f"Değişken {i+1}" for i in range(len(importances))]
    ...
    # 4. Tablo Bölümü (Hücreler Paragraph ile sarılarak Türkçe font garanti edilir)
    ...
    table_data = [[
        Paragraph("Model Adı", header_style),
        Paragraph("Durum", header_style),
        Paragraph("Hesaplanan Metrikler", header_style),
        Paragraph("Süre (sn)", header_style),
        Paragraph("Tekrar", header_style)
    ]]

    for t in tasks:
        metrics_str = f"MSE: {t.metrics.get('mse', 0):.2f}, R2: {t.metrics.get('r2', 0):.4f}" if t.metrics else "N/A"
        table_data.append([
            Paragraph(str(t.model_name), cell_style),
            Paragraph(str(t.status), cell_style),
            Paragraph(metrics_str, cell_style),
            Paragraph(f"{t.execution_time:.2f} s" if t.execution_time else "-", cell_style),
            Paragraph(str(t.retry_count), cell_style)
        ])
    ...
    # 5. Canlı Tahmin Kutusu
    story.append(Paragraph("Otomatik Sistem Değerlendirmesi ve Tahmin Servisi", section_heading))
    eval_text = f"Eğitilen <b>{len(tasks)}</b> adet model arasından en yüksek başarı oranını (R²: <b>{best_r2:.4f}</b>) gösteren <b>{best_model}</b> modeli seçilmiştir.<br/><br/><b>Canlı Tahmin Durumu:</b> Seçilen model <code>saved_models/{job.id}_{best_model}.joblib</code> yoluna başarıyla serileştirilmiştir."
...
@router.get("/reports/{job_id}/pdf", tags=["AutoML Reports"])
def get_pdf_report(job_id: str, db: Session = Depends(get_db)):
    job = db.query(TrainingJob).filter(TrainingJob.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="İş (Job) bulunamadı.")
    
    tasks = db.query(ModelTaskResult).filter(ModelTaskResult.job_id == job_id).all()
    
    pdf_bytes = generate_pdf_report(job, tasks)
    
    return Response(
        content=pdf_bytes,
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=AutoML_Report_{job_id}.pdf"}
    )